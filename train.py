...
#         self.weights = nn.Parameter(torch.randn(4))  # Learnable weights
...
#         total = w[0]*l_charb + w[1]*l_perc + w[2]*l_grad + w[3]*l_ssim
#         return total
...
def train():
    ...
    optimizer_b = optim.AdamW(
        list(model.parameters()) + list(loss_fn.parameters()),  # ✅ include learnable loss weights
    ...
    if opt.TRAINING.RESUME and opt.TRAINING.WEIGHT is not None:
        ...
        if 'optimizer' in checkpoint and 'epoch' in checkpoint:
            ...
            if 'loss_weights' in checkpoint:
                loss_fn.load_state_dict(checkpoint['loss_weights'])

        print(f"✅ Resumed from epoch {start_epoch - 1}")

    trainloader, testloader = accelerator.prepare(trainloader, testloader)
    model, optimizer_b, scheduler_b, loss_fn = accelerator.prepare(model, optimizer_b, scheduler_b, loss_fn)
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            # Log loss weights
            if accelerator.is_local_main_process:
                weight_vals = loss_fn.softmax(loss_fn.weights).detach().cpu().numpy()
                accelerator.log({
                    "Loss Weight - Charbonnier": weight_vals[0],
                    "Loss Weight - Perceptual": weight_vals[1],
                    "Loss Weight - Gradient": weight_vals[2],
                    "Loss Weight - MS-SSIM": weight_vals[3]
                }, step=epoch)
            ...
            if psnr > best_psnr:
                ...
                save_checkpoint({
                    ...
                    'optimizer': optimizer_b.state_dict(),
                    'scheduler': scheduler_b.state_dict(),
                    'loss_weights': loss_fn.state_dict(),  # ✅ Save loss weights
                    'best_psnr': best_psnr,
                    'best_psnr_epoch': best_psnr_epoch
                }, epoch, opt.MODEL.SESSION, opt.TRAINING.SAVE_DIR)
            else:
