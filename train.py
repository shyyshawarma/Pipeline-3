...
def train():
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        if accelerator.is_local_main_process:
            soft_weights = loss_fn.softmax(loss_fn.weights).detach().cpu().numpy()
            print(f"📊 Epoch {epoch} Adaptive Loss Weights: "
                  f"Charbonnier: {soft_weights[0]:.4f}, "
                  f"Perceptual: {soft_weights[1]:.4f}, "
                  f"Gradient: {soft_weights[2]:.4f}, "
                  f"MS-SSIM: {soft_weights[3]:.4f}")
